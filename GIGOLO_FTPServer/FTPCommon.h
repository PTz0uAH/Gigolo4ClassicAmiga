...
#define BUFFERSIZE 4096//CONFIG_TCP_MSS
#define PRINTu32 "u"
#endif

#define FTP_SERVER_VERSION "0.9.7-20210722"

#define FTP_CTRL_PORT 21         // Command port on which server is listening
#define FTP_DATA_PORT_PASV 20    // Data port in passive mode CLASSIC AMIGA??
#define FTP_TIME_OUT 123         // Disconnect client after 123 minutes of inactivity
#define FTP_CMD_SIZE 127         // allow max. 127 chars in a received command

// Use ESP Core Debug functionality
#ifdef DEBUG_ESP_PORT
#define FTP_DEBUG_MSG(fmt, ...)                                          \
    do                                                                   \
    {                                                                    \
        DEBUG_ESP_PORT.printf("[FTP] " fmt "\n", ##__VA_ARGS__); \
        yield();                                                         \
    } while (0)
#else
...
//AMIGODOS DOPUS DOCK CUSTOM COMMANDS FLOPPY CONTROL VIA FTP
//AMIGODOS EXTENDED COMMANDS
//EXEC - execute AmiGoDOS Script from "BIN/S" folder
#define FTP_CMD_LE_ADOS 0x534F4441      // "ADOS" as uint32_t (little endian)
#define FTP_CMD_BE_ADOS 0x41444F53      // "ADOS" as uint32_t (big endian)
#define FTP_CMD_LE_DISK 0x4B534944      // "DISK" as uint32_t (little endian)
#define FTP_CMD_BE_DISK 0x4449534B      // "DISK" as uint32_t (big endian)
/*
#define FTP_CMD_LE_EXEC 0x43455845      // "EXEC" as uint32_t (little endian)
#define FTP_CMD_BE_EXEC 0x45584543      // "EXEC" as uint32_t (big endian)
#define FTP_CMD_LE_GRAB 0x42415247      // "GRAB" as uint32_t (little endian)
#define FTP_CMD_BE_GRAB 0x47524142      // "GRAB" as uint32_t (big endian)
#define FTP_CMD_LE_WRIT 0x54495257      // "WRIT" as uint32_t (little endian)
#define FTP_CMD_BE_WRIT 0x57524954      // "WRIT" as uint32_t (big endian)
#define FTP_CMD_LE_LOAD 0x44414F4C      // "LOAD" as uint32_t (little endian)
#define FTP_CMD_BE_LOAD 0x4C4F4144      // "LOAD" as uint32_t (big endian)
#define FTP_CMD_LE_SAVE 0x45564153      // "SAVE" as uint32_t (little endian)
#define FTP_CMD_BE_SAVE 0x53415645      // "SAVE" as uint32_t (big endian)
*/
#define FTP_CMD_LE_TEST 0x54534554      // "TEST" as uint32_t (little endian)
#define FTP_CMD_BE_TEST 0x54455354      // "TEST" as uint32_t (big endian)

class FTPCommon
{
public:
    // contruct an instance of the FTP Server or Client using a
    // given FS object, WE USE SD_MMC //e.g. SPIFFS or LittleFS
    ...
    void setTimeout(uint32_t timeoutMs = (uint32_t)FTP_TIME_OUT * 60 * 1000);
